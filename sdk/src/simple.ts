...
import { Loop, CaptureType, ReleaseCondition, PermissionLevel, Vault } from './index';
...
export class LoopSDK {
   ...
   */
  async getOrCreateVault(owner?: PublicKey): Promise<{ address: PublicKey; vault: Vault | null }> {
   ...
   */
  async getTotalVaultValue(owner?: PublicKey): Promise<number> {
    const vaultOwner = owner || this.wallet.publicKey;
    const vault = await this.loop.vault.getVault(vaultOwner);
    if (!vault) return 0;
   ...
   */
  async capture(params: CaptureParams): Promise<string> {
   ...
   */
  async stack(params: StackParams): Promise<string> {
   ...
   */
  async revokeAgentPermission(agent: PublicKey): Promise<string> {
    const owner = this.wallet.publicKey;
   ...
   */
  async agentStack(vaultOwner: PublicKey, amount: number, durationDays: number): Promise<string> {
   ...
   */
  async agentUnstack(vaultOwner: PublicKey, stackAddress: PublicKey): Promise<string> {
    const agent = this.wallet.publicKey;
   ...
   */
  async agentRebalance(vaultOwner: PublicKey, targetStackRatio: number): Promise<string> {
    const agent = this.wallet.publicKey;
  ...
  async createEscrow(params: CreateEscrowParams): Promise<string> {
    ...
    const releaseConditions: ReleaseCondition[] = params.conditions.map(c => {
      if (c.type === 'arbiter') {
        const arbiter = c.arbiter || c.arbiterPubkey;
        if (!arbiter) throw new Error('Arbiter condition requires arbiter pubkey');
        return { arbiterApproval: { arbiter } };
      }
      if (c.type === 'time') {
        const timestamp = c.timestamp || c.deliveryDeadline;
        if (!timestamp) throw new Error('Time condition requires timestamp');
   ...
   */
  async transfer(recipient: PublicKey, amount: number, memo?: string): Promise<string> {
    console.log(`[Loop] Transfer: ${amount} Cred to ${recipient.toBase58()}`);
    if (memo) console.log(`[Loop] Memo: ${memo}`);
